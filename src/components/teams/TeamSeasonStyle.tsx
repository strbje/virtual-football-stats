...
export default function TeamSeasonStyle({ profile }: { profile: SeasonProfile }) {
  ...
  return (
    <section className="mt-4 rounded-xl border border-zinc-200 p-3 text-xs space-y-3 bg-white">
      ...
      {/* Атака */}
      <div>
        <div className="font-semibold mb-1">🎯 Атака</div>
        <ul className="space-y-0.5">
          <li>
            Голы — {p.goalsPerMatch.toFixed(2)} за матч ({p.goalsTotal} / {p.matches})
          </li>
          <li>
            xG — {p.xgPerMatch.toFixed(2)} за матч ({p.xgTotal.toFixed(1)} / {p.matches})
          </li>
          <li>Удары — {p.shotsPerMatch.toFixed(2)} за матч</li>
          <li>Удары в створ — {p.shotsOnTargetPerMatch.toFixed(2)} за матч</li>
          <li>
            Точность ударов —{" "}
            {p.shotsAccuracyPct !== null ? `${p.shotsAccuracyPct.toFixed(1)}%` : "—"}
          </li>
          <li>
            Пасов на удар —{" "}
      ...
      <div>
        <div className="font-semibold mb-1">⚡ Созидание и владение</div>
        <ul className="space-y-0.5">
          <li>
          ...
          <li>
            Точность паса —{" "}
            {p.passAccuracyPct !== null ? `${p.passAccuracyPct.toFixed(1)}%` : "—"}
          </li>
          <li>
            xA — {p.xaPerMatch.toFixed(2)} за матч ({p.xaTotal.toFixed(1)} / {p.matches})
          </li>
          <li>
            pXA —{" "}
            {p.pxa !== null ? p.pxa.toFixed(2) : "—"}{" "}
            — среднее количество пасов на 0.5 xA
          </li>
        </ul>
      </div>
      ...
      {/* Фланги и навесы */}
      <div>
        <div className="font-semibold mb-1">🌪 Фланги и навесы</div>
        <ul className="space-y-0.5">
          <li>
            Навесы — {p.crossesPerMatch.toFixed(2)} за матч
          </li>
          <li>
          ...
          <li>
            Точность навесов —{" "}
            {p.crossesAccuracyPct !== null ? `${p.crossesAccuracyPct.toFixed(1)}%` : "—"}
          </li>
        </ul>
      </div>
      ...
      {/* Оборона и воздух */}
      <div>
        <div className="font-semibold mb-1">🛡 Оборона и воздушные дуэли</div>
        <ul className="space-y-0.5">
          <li>
          ...
          <li>
            ...
            {p.aerialWinPct !== null ? `${p.aerialWinPct.toFixed(1)}%` : "—"}
          </li>
        </ul>
      </div>
    </section>
