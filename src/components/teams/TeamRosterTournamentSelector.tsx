  ...
  return (
    <div className="relative inline-block w-full">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between rounded-xl border border-zinc-200 bg-zinc-50/60 px-3 py-2 text-xs text-zinc-700"
      >
        ...
        <div className="absolute z-20 mt-1 w-full rounded-xl border border-zinc-200 bg-white p-2 shadow-lg">
          <div className="max-h-64 overflow-y-auto space-y-1 text-xs">
            {sorted.map((t) => {
              ...
              // подпись: оригинальное название, а сезон/лига — мелким шрифтом
              const seasonLabel =
              ...
              return (
                <label
                  key={t.id}
                  className="flex cursor-pointer items-start gap-2 rounded-lg px-2 py-1 hover:bg-zinc-50"
                  title={t.name}
                >
                  <input
                    type="checkbox"
